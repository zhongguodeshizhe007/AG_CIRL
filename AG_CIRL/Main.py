...
from model.discriminator import DiscriminatorCIRL
...
def update_params_airl(batch, i_iter): 
    ...
    for _ in range(epoch_disc): # epoch_disc = 1
        ...
        loss_pi = -F.logsigmoid(-g_o).mean() # F.logsigmoid(): 计算输入张量的元素的 log-sigmoid 函数值
    ...
    optim_iter_num = int(math.ceil(states.shape[0] / optim_batch_size)) # optim_batch_size=64
    for _ in range(optim_epochs): # optim_epochs=10
        value_loss, policy_loss = 0, 0
...
def main_loop():
    best_edit = 1.0
    for i_iter in range(1, max_iter_num + 1): # max_iter_num=1000/2000/3000
        # load_model(model_p)
        """generate multiple trajectories that reach the minimum batch_size""" # 产生多条达到最小批量的轨迹
        discrim_net.to(torch.device('cpu'))
        discrim_net.to_device(torch.device('cpu'))
        batch, log = agent.collect_samples(min_batch_size, mean_action=False) # 返回的batch包括('state', 'destination', 'action', 'next_state', 'reward', 'mask', 'bad_mask')，每个元素都很长很长
        ...
        train_trajs1 = env.import_demonstrations_step(train_p)
        log_prob = evaluate_log_prob(train_trajs1, policy_net)
        edit_dist, bleu_score, js_dist = evaluate_model(test_od, test_trajs, policy_net, env) # 学习四个指标
        edit_dist0.append(edit_dist)
        bleu_score0.append(bleu_score)
        js_dist0.append(js_dist)
        log_prob0.append(log_prob)
        if i_iter%50 == 0:
            print(i_iter)
    # 保存指标数据
    import pandas as pd
    # 创建一个DataFrame
    data = {'edit_dist': edit_dist0, 'bleu_score': bleu_score0, 'js_dist': js_dist0, 'log_prob': log_prob0}
    df = pd.DataFrame(data)
    # 指定CSV文件名
    csv_filename = "../four_metrics/airl_CV%d_size%d.csv" % (cv, size)
    # 将DataFrame保存为CSV文件
...
if __name__ == '__main__':
    ...
    network_p = "../data/transit.npy" # 转移矩阵：
    path_feature_p = "../data/feature_od.npy" # context特征𝐹𝑐
    ...
    path_feature = minmax_normalization(path_feature, path_max, path_min) # 最大最小归一化
    ...
    policy_net = Policy(env.n_actions, env.policy_mask, 
                           env.state_action, path_feature_pad, edge_feature_pad, 
                           path_feature_pad.shape[-1] + edge_feature_pad.shape[-1] + 1, 
                           env.pad_idx).to(device) # 714
    value_net = Value(path_feature_pad, edge_feature_pad, 
                         path_feature_pad.shape[-1] + edge_feature_pad.shape[-1]).to(device) # 
    discrim_net = DiscriminatorCIRL(env.n_actions, gamma, env.policy_mask, 
                                       env.state_action, path_feature_pad, edge_feature_pad, #
                                       path_feature_pad.shape[-1] + edge_feature_pad.shape[-1] + 1, 
    ...
    # train_traj, train_od = load_test_traj(train_p) # 为了学习四个指标
    edit_dist0 = []
    bleu_score0 = []
    js_dist0=[]
    log_prob0 = []
    ...
    start_time = time.time()
    edit_dist, bleu_score, js_dist = evaluate_model(test_od, test_trajs, policy_net, env)
    ...
    log_prob = evaluate_log_prob(test_trajs, policy_net)
    print(log_prob)
